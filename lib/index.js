    ...
    return readdirSync(path.resolve(homedir(), './.aws/sso/cache'))
        .reduce(
            (token, file) => {
                ...
                if (date > new Date() && typeof data.accessToken === 'string' && data.accessToken !== '') {
...
export default async (profile) => {
    ...
    if (creds[profile].expiration !== undefined && creds[profile].expiration > Date.now()) {
        return;
    }

    const login = (profile) => new Promise((res, rej) => {
